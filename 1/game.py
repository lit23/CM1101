...
def normalise_input(user_input):
    ...
    'help'
    """
    rpunct = remove_punct(user_input)
    rspace = remove_spaces(rpunct)
    lcase = rspace.lower()
...
def print_menu(exits):
    ...
    print("You can:")
    for x in exits:
        print_menu_line(x, rooms[exits[x]]["name"])
...
def menu(exits):
    ...
    while True:
        ...
        player_choice = input()
        # Read player's input
        normalised_input = normalise_input(player_choice)
        # Normalise the input
        TF = is_valid_exit(exits, normalised_input)
        if TF == True:
