...
def filter_words(words, skip_words):
    ...
    for important_words in words:
        if important_words not in skip_words:
            output_words.append(important_words) 
...
def normalise_input(user_input):
    ...
    for original in user_input:
        if original not in punctuations:
            output_string = output_string + original
            
    case_space = output_string.strip().split()

    fwords = filter_words(case_space, skip_words)
